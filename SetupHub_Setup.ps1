...
.NOTES
    ...
    Some packages, including Microsoft 365 Apps / Office, may require a valid license/account. v1.0 is the first stable SetupHub release: catalog validation and hardware/software inventory are mandatory, custom profile creation is included, unavailable packages are skipped, and non-reliable packages are moved to a Manual/Unsupported section in the report.
#>
...
$script:Text = @{
    it = @{
        ...
        Footer = 'Creato da Pietro Melillo | Powered by WinGet | SetupHub v1.0'
    }
    en = @{
        ...
        Footer = 'Created by Pietro Melillo | Powered by WinGet | SetupHub v1.0'
    }
}
...
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
    ...
    <Grid>
        ...
        <StackPanel Grid.Row="2" Orientation="Horizontal" HorizontalAlignment="Center" Margin="0,6,0,8">
            <StackPanel Orientation="Vertical" Margin="0,0,20,0" VerticalAlignment="Center">
                <CheckBox x:Name="chkReport" Content="Genera report HTML/CSV" Foreground="#cdd6f4" IsChecked="True"/>
            </StackPanel>
        ...
        <Border Grid.Row="4" Background="#181825" Padding="10,6">
            <TextBlock x:Name="lblFooter" Text="Creato da Pietro Melillo | Powered by WinGet | SetupHub v1.0" Foreground="#6c7086" FontSize="10" HorizontalAlignment="Center"/>
        </Border>
    </Grid>
</Window>
"@
...
$btnStart.Add_Click({
    ...
    $btnCancel.Content = T 'Cancel'
    foreach ($control in @($btnSelectAllInstall,$btnDeselectAllInstall,$btnSelectAllBloat,$btnDeselectAllBloat,$btnResetRecommended,$btnNewProfile,$btnApplyProfile,$btnSaveProfile,$btnLoadProfile,$cmbProfile,$cmbLanguage,$chkReport)) { $control.IsEnabled = $false }
    ...
    $psCmd.AddScript({
        ...
        function Test-WinGetNotInstalledMessage {
            param([string]$Message)
            return ($Message -match '(?i)No installed package found matching input criteria|No installed package found|Nessun pacchetto installato|not installed|not found matching input criteria')
        }
        ...
        if ($createReport) {
            ...
            $changelogIt = @(
                [pscustomobject]@{ Version='1.0'; Language='IT'; Changes='Prima release stabile: catalogo software validato automaticamente; inventario hardware/software obbligatorio; profili personalizzati; report HTML/CSV/JSON; gestione bloatware Windows 10/11; sezione pacchetti manuali/non supportati; credits e licenza.' }
            )
            $changelogEn = @(
                [pscustomobject]@{ Version='1.0'; Language='EN'; Changes='First stable release: automatic software catalog validation; mandatory hardware/software inventory; custom profiles; HTML/CSV/JSON reports; Windows 10/11 bloatware handling; manual/unsupported package section; credits and license.' }
            )
